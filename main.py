...
from system.checks import run_preliminary_checks, detect_existing_installations
...
def show_main_menu(installations):
    ...
    for i, (option, desc) in enumerate(zip(options, descriptions), 1):
        status = ""
        if i <= 5:  # First 5 are component-specific
...
def main():
    ...
    try:
        ...
        log_step("Detecting Existing Installations")
        installations = detect_existing_installations()
        
        # Show detection results
        print("\nSystem Status:")
        print(f"  NVIDIA Driver: {'✓ ' + installations['nvidia_driver']['version'] if installations['nvidia_driver']['installed'] else '✗ Not installed'}")
        print(f"  Docker:        {'✓ ' + installations['docker']['version'] if installations['docker']['installed'] else '✗ Not installed'}")
        print(f"  NVIDIA Runtime: {'✓ Available' if installations['nvidia_runtime']['installed'] else '✗ Not configured'}")
        print()
        
        # Interactive menu loop
        while True:
            options = show_main_menu(installations)
