...
def show_docker_info():
    ...
    try:
        ...
        for line in lines:
            if any(keyword in line.lower() for keyword in ['runtime', 'storage', 'cgroup']):
                log_info(f"  {line.strip()}")
                
    except Exception as e:
        log_info(f"Could not retrieve Docker info: {e}")


def validate_nvidia_docker():
    ...
    try:
        ...
        if "NVIDIA-SMI" in output:
            ...
            log_info("✗ NVIDIA Docker integration test failed")
            return False
            
    except Exception as e:
        log_info(f"✗ NVIDIA Docker test failed: {e}")
        return False